	...
	$req_packages = [ "gcc", "make", "freetype2*", "curl", "libfcgi-dev", "libfcgi0ldbl", "libjpeg62-dbg", "libmcrypt-dev", "libssl-dev", "libjpeg-dev", "libfreetype6-dev", "libxpm-dev", "libxml2-dev", "libpcre3-dev", "libbz2-dev", "libcurl4-openssl-dev", "libpng12-dev", "libmysqlclient-dev", "libt1-dev", "libgd2-xpm-dev", "libgmp-dev", "libsasl2-dev", "libmhash-dev", "unixodbc-dev", "freetds-dev", "libpspell-dev", "libsnmp-dev", "libtidy-dev", "libxslt1-dev", "libdb5.3-dev" ]
        ...
        file {
           'check_php_version':
              ...
              content => template('php/check_php_version.sh.erb'),
              path => '/tmp/check_php_version.sh',
              owner => 'root',
              group => 'root',
              mode  => '0744',
         }
        ...
        exec {
           'run_check_php_version_script':
           require => File['check_php_version'],
           command => "bash -c \"/tmp/check_php_version.sh ${php_version}\"",
        }

        package {$req_packages:
          ensure => "installed",
	  before => Exec['download_php'],
        #  require => Exec['run_check_php_version_script'],
        }
...
#	}
	...
	exec { "install_apc":
	    ...
	    unless => "find /opt/PHP-${php_version} -iname apc.so | egrep '.*'"
	}
	...
	exec { "install_memcache":
            ...
            cwd => "/opt/PHP-${php_version}/bin",
            #command => "ls",
            command => "bash pecl install memcache-3.0.6.tgz",
            unless => "find /opt/PHP-${php_version} -iname memcache.so | egrep '.*'"
        }
	...
	service { "php-fpm":
        ...
        file {
           'install_php_script':
              ...
              content => template('php/install_php_script.sh.erb'),
              path => '/tmp/install_php_script.sh',
              owner => 'root',
              group => 'root',
              mode  => '0755',
         }
        ...
        exec {
           'run_install_php_script':
           require => File['install_php_script'],
           command => "/tmp/install_php_script.sh ${php_version}",
           logoutput => true,
        }
